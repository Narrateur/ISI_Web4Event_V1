		...
		if($animation != NULL) {
			...
			foreach($animation as $ani){
				if($ani['etat']=='en cours'){
					...
					echo"
						<tr>
						<th scope='row'> <input type='text' value=".$ani["ani_id"]." readonly='readonly' id='ani_id'/> <input type='text' id='ani_libelle' value='".$ani["ani_libelle"]."'></th>
						
                        <td> <textarea type='text' id='ani_description' >".$ani["ani_description"]."</textarea><br>
                        <h5 class='mt-3 mb-3'>Où? <a href='".base_url()."index.php/lieu/afficher/".$ani["lie_id"]."'>".$ani["lie_libelle"]."</a></h5><br>
                        Quand? Du ".$ani["ani_horaireDebut"]." au ".$ani["ani_horaireFin"]."</td>
						
                        <td><h3 class='mt-3 mb-3'><a href='".base_url()."index.php/invite/galerie_animation/".$ani["ani_id"]."'>".$ani["invite"]."</a></h3></td>
                        
                        <td><input type='submit' value='Mettre à Jour'/><br><input type='submit' value='Supprimer'/></td>
						</tr>";
				}
			}
